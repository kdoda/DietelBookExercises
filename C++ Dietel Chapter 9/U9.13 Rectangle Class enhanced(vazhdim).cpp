...
void setBorderCharacter(char c){
    borderC=c;
...
int amin=min(min(a1,a2),min(a3,a4));  //sepse duhet te gjejme kulmin me te vogel qe te fillojm nga atje nr ta kemi si referenc
int bmin=min(min(b1,b2),min(b3,b4));

for(int i=0;i<25;i++){
    for(int j=0;j<25;j++)
    {
        if(bmin<=i && i<bmin+width && amin<=j && j<amin+length)
            {
        if(i==bmin||i==width-1+bmin ||j==amin||j==amin+length-1)
            cout<<borderC;
        else cout<<fillC;
            }
            else cout<<" ";
...
Rectangle r;
r.setCoordinates(3,3,3,10,15,3,15,10);
